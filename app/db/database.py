...
class DataBase():
    ...
    def read_tasks(self, id_user: int):
        try:
            ...
            for result in results:
                results_return.append(TaskFastapi(idTask=result.idTask, title=result.title, description=result.description))
    ...
    def update_task(self, id_task: int, id_user: int, update: TaskUpdate):
        try:
            results = self.session.query(TaskDb).filter(TaskDb.idTask == id_task, TaskDb.idUser == id_user).first()
            if results:
    ...
    def delete_task(self, id_task: int, id_user: int):
        try:
            results = self.session.query(TaskDb).filter(TaskDb.idTask == id_task, TaskDb.idUser == id_user).delete()
            if results == 0:
                raise Exception("The task not exists")
            verify_delete = self.session.query(TaskDb).filter(TaskDb.idTask == id_task, TaskDb.idUser == id_user).first()
            if verify_delete:
